#!/usr/bin/env python3
"""
Anthropic Claude Chat Agent – Production-Ready Chat Interface

Cette application fournit une implémentation complète d'un agent de chat utilisant les modèles d'Anthropic
en utilisant leur API de completions. Les fonctionnalités incluent :
...
@dataclass
class AgentConfig:
    """Paramètres de configuration pour l'agent de chat Claude d'Anthropic"""
    model: str = "claude-sonnet-4-20250514"  # Modèle par défaut de Claude
    temperature: float = 1.0
    max_tokens: Optional[int] = 64000  # Valeur par défaut ajustée à 64000
    max_history_size: int = 1000
    stream: bool = True
    system_prompt: Optional[str] = "You are an agent."
    response_format: str = "json"  # Format de sortie souvent en JSON pour les API
    top_p: float = 1.0
...
class AnthropicClaudeChatAgent:
    """Agent de chat Claude d'Anthropic avec persistance et support du streaming"""

    SUPPORTED_MODEL = {
        "claude-sonnet-4-20250514": {
            "name": "Claude Sonnet 4",
            "description": "Modèle Claude Sonnet 4 d'Anthropic avec capacité contextuelle étendue",
            "timeout": 600,  # 10 minutes pour les très longues réponses
            "max_output_tokens": 64000  # Capacité maximale de tokens de sortie
        },
    ...
    def _make_api_request(self, payload: Dict[str, Any]) -> requests.Response:
        ...
        headers = {
            ...
            "anthropic-beta": "context-1m-2025-08-07"  # Ajout de l'en-tête beta pour 1 million de contexte
        }
    ...
    def call_api(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Generator[str, None, None]:
        """Appeler l'API Claude d'Anthropic avec le nouveau message"""
        try:
...
"""
    ...
    def interactive_chat(self):
        ...
        print(f"\n{Fore.GREEN}Démarrage du chat interactif avec Claude d'Anthropic")
            ...
            try:
                ...
                if user_input.startswith('/'):
                    ...
                    if command == 'help':
                        ...
                        print(f"{Fore.WHITE}/history [n] - Afficher les derniers n messages (par défaut 5)")
                        print(f"{Fore.WHITE}/search <terme> - Rechercher dans l'historique de conversation")
                        print(f"{Fore.WHITE}/stats - Afficher les statistiques de la conversation")
                        print(f"{Fore.WHITE}/config - Afficher la configuration actuelle")
                        print(f"{Fore.WHITE}/export <json|txt|md|html> - Exporter la conversation")
                        print(f"{Fore.WHITE}/clear - Effacer l'historique de conversation")
                        print(f"{Fore.WHITE}/files - Lister les fichiers disponibles pour inclusion")
                        print(f"{Fore.WHITE}/info - Afficher les informations de l'agent")
                        print(f"{Fore.WHITE}/quit - Quitter le chat{Style.RESET_ALL}\n")
    ...
    def main():
        ...
        parser = argparse.ArgumentParser(
            description="Anthropic Claude Chat Agent - Advanced AI Chat Interface",
