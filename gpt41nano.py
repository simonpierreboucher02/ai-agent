#!/usr/bin/env python3  
"""  
OpenAI GPT-4.1 Nano Chat Agent - Production-Ready Chat Interface

Ce module fournit une implémentation complète d'un agent de chat pour le modèle GPT-4.1-nano  
...
Exemples d'utilisation :  
    ...
    python openai_gpt41_nano_agent.py --agent-id my-agent

    # Lister tous les agents  
    python openai_gpt41_nano_agent.py --list

    # Exporter la conversation  
    python openai_gpt41_nano_agent.py --agent-id my-agent --export html

    # Configurer les paramètres de l'agent  
    python openai_gpt41_nano_agent.py --agent-id my-agent --config  
"""
...
try:  
    ...
    class Fore:  
        RED = BLUE = GREEN = YELLOW = CYAN = WHITE = RESET = ""  
...
@dataclass  
class AgentConfig:  
    """Configuration settings for the OpenAI GPT-4.1 Nano Chat Agent"""  
...
class OpenAIGPT41NanoChatAgent:  
    """OpenAI GPT-4.1 Nano Chat Agent with persistence and streaming support"""
    ...
    def __init__(self, agent_id: str):  
        ...
        self.logger.info(f"Initialisé l'Agent de Chat OpenAI GPT-4.1 Nano : {agent_id}")
    ...
    def _setup_logging(self):  
        ...
        # Créer le logger  
        self.logger = logging.getLogger(f"OpenAIGPT41NanoAgent_{self.agent_id}")  
    ...
    def _get_api_key(self) -> str:  
        ...
        print(f"{Fore.YELLOW}Clé API non trouvée pour le modèle OpenAI GPT-4.1 Nano.")  
        print(f"Vous pouvez définir la variable d'environnement OPENAI_API_KEY ou la saisir maintenant.{Style.RESET_ALL}")

        api_key = input(f"{Fore.CYAN}Entrez la clé API pour OpenAI GPT-4.1 Nano : {Style.RESET_ALL}").strip()
    ...
    def _make_api_request(self, payload: Dict[str, Any]) -> requests.Response:  
        ...
        self.logger.info(f"Utilisation d'un timeout de {timeout}s pour {self.SUPPORTED_MODEL[self.config.model]['name']}")
        ...
        for attempt in range(max_retries):  
            try:  
                self.logger.info(f"Envoi de la requête API (tentative {attempt + 1}/{max_retries}) avec un timeout de {timeout}s...")
    ...
    def _parse_streaming_response(self, response: requests.Response) -> Generator[str, None, None]:  
        """Analyser la réponse en streaming"""  
        assistant_message = ""
    ...
    def call_api(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Generator[str, None, None]:  
        """Appeler l'API OpenAI GPT-4.1-nano avec le nouveau message"""  
        try:  
            ...
            print(f"{Fore.YELLOW}🤖 Utilisation de {model_display} (timeout : {timeout//60}m {timeout%60}s)...{Style.RESET_ALL}")
    ...
    def export_conversation(self, format_type: str) -> str:  
        ...
        if format_type == "json":  
            ...
            with open(filepath, 'w', encoding='utf-8') as f:  
                f.write(f"Exportation de Conversation - OpenAI {model_display} Chat Agent\n")  
            ...
            with open(filepath, 'w', encoding='utf-8') as f:  
                f.write(f"# Conversation OpenAI {model_display} Chat Agent\n\n")  
                f.write(f"**Agent ID:** {self.agent_id}  \n")  
                f.write(f"**Modèle:** {self.config.model}  \n")  
                f.write(f"**Exporté le:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  \n\n")
            ...
            # Template HTML avec style amélioré et support des blocs de code
            html_template = f"""<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Conversation OpenAI {model_display} - {self.agent_id}</title>
    <style>
        body {{
...
<body>
    <div class="container">
        <div class="header">
            <h1>Conversation OpenAI {model_display} Chat Agent</h1>
            <p>Agent ID : {self.agent_id}</p>
            <p>Exporté le : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
        </div>

        <div class="stats">
            <h2>Statistiques de la Conversation</h2>
            <table>
                <tr><th>Total Messages</th><td>{stats['total_messages']}</td></tr>
                <tr><th>Messages Utilisateur</th><td>{stats['user_messages']}</td></tr>
                <tr><th>Messages Assistant</th><td>{stats['assistant_messages']}</td></tr>
                <tr><th>Total Caractères</th><td>{stats['total_characters']:,}</td></tr>
                <tr><th>Longueur Moyenne des Messages</th><td>{stats['average_message_length']:,}</td></tr>
                <tr><th>Premier Message</th><td>{stats['first_message']}</td></tr>
                <tr><th>Dernier Message</th><td>{stats['last_message']}</td></tr>
                <tr><th>Durée</th><td>{stats['conversation_duration']}</td></tr>
            </table>
        </div>
...
"""
            ...
            for msg in self.messages:  
                ...
                # Échapper le contenu pour HTML
                content_escaped = self._escape_html(content)

                # Gérer les blocs de code délimités par ```
                content_formatted = self._format_code_blocks(content_escaped)
...
"""
            ...
            html_template += f"""    </div>
    <div class="footer">
        <p>Exporté par OpenAI {model_display} Chat Agent</p>
    </div>
</div>
</body>
</html>"""
            ...
            if format_type == "html":  
                try:
                    ...
                    self.logger.error(f"Erreur lors de l'exportation en HTML : {e}")
                    raise e
    ...
    def _format_code_blocks(self, text: str) -> str:
        """Formater les blocs de code délimités par ``` en HTML"""
        parts = re.split(r'```(.*?)```', text, flags=re.DOTALL)
        formatted = ""
        for i, part in enumerate(parts):
            if i % 2 == 1:
                # Partie de code
                formatted += f'<pre><code>{part}</code></pre>'
            else:
                # Texte normal
                formatted += part
        return formatted
            ...
            if config_file.exists():  
                try:  
                    with open(config_file) as f:  
                        ...
                        agent_info["model"] = config.get("model", "gpt-4.1-nano")  
...
def show_agent_info(agent_id: str):  
    ...
    if config_file.exists():  
        try:  
            ...
            model_display = OpenAIGPT41NanoChatAgent.SUPPORTED_MODEL.get(model, {}).get('name', model)
...
def interactive_chat(agent: OpenAIGPT41NanoChatAgent):  
    """Session de chat interactive"""  
    print(f"\n{Fore.GREEN}Démarrage du chat interactif avec OpenAI {agent.SUPPORTED_MODEL.get(agent.config.model, {}).get('name', agent.config.model)}")  
                ...
                if command == 'help':  
                    ...
                    if not recent_messages:  
                        ...
                        for msg in recent_messages:  
                            ...
                            preview = msg["content"][:100] + "..." if len(msg["content"]) > 100 else msg["content"]  
                            print(f"{Fore.WHITE}[{timestamp}] {role_color}{msg['role']} : {preview}")  
                    print()
                    ...
                    print(f"{Fore.WHITE}Modèle : {agent.config.model} ({agent.SUPPORTED_MODEL.get(agent.config.model, {}).get('name', agent.config.model)})")  
                    ...
                    for key, value in config_dict.items():  
                        if key not in ['created_at', 'updated_at']:  
                            if key == 'model':  
                                model_name = agent.SUPPORTED_MODEL.get(str(value), {}).get('name', value)  
...
def main():  
    ...
    parser = argparse.ArgumentParser(  
        description="OpenAI GPT-4.1 Nano Chat Agent - Interface Avancée de Chat IA",  
        formatter_class=argparse.RawDescriptionHelpFormatter,  
        epilog="""  
Exemples :  
  %(prog)s --agent-id my-agent                    # Démarrer un chat interactif  
  %(prog)s --list                                 # Lister tous les agents  
  %(prog)s --agent-id my-agent --export html      # Exporter la conversation en HTML  
  %(prog)s --agent-id my-agent --config          # Configurer l'agent de manière interactive  
        """  
    )
    ...
    if args.list:  
        ...
        print(f"{Fore.WHITE}{'ID':<20} {'Modèle':<20} {'Messages':<10} {'Dernière Mise à Jour':<20}")  
        print("-" * 70)
        ...
        for agent in agents:  
            ...
            model_display = OpenAIGPT41NanoChatAgent.SUPPORTED_MODEL.get(model, {}).get('name', model)  
            print(f"{agent['id']:<20} {model_display:<20} {agent.get('message_count', 0):<10} {updated:<20}")
    ...
    try:  
        # Initialiser l'agent  
        agent = OpenAIGPT41NanoChatAgent(args.agent_id)
