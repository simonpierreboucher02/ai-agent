#!/usr/bin/env python3
"""
OpenAI O3 Chat Agent - Advanced Production-Ready Chat Interface

This module provides a complete chat agent implementation for OpenAI's O3 model
using the standard chat completions API endpoint. Features include:

- Support for O3 model with reasoning capabilities
...
Example Usage:
    ...
    python openai_o3_agent.py --agent-id my-agent

    # List all agents
    python openai_o3_agent.py --list

    # Export conversation
    python openai_o3_agent.py --agent-id my-agent --export html

    # Configure agent settings
    python openai_o3_agent.py --agent-id my-agent --config
"""
...
@dataclass
class AgentConfig:
    """Configuration settings for the OpenAI O3 Chat Agent"""
    ...
    @staticmethod
    def get_timeout_for_reasoning(reasoning_effort: str = "medium") -> int:
        """Get appropriate timeout based on reasoning effort level"""
        # O3 model timeouts
        reasoning_timeout = {
            "low": 240,    # 4 minutes
        ...
        return reasoning_timeout.get(reasoning_effort, 600)


class OpenAIO3ChatAgent:
    """Advanced OpenAI O3 Chat Agent with persistence and streaming support"""
    ...
    def __init__(self, agent_id: str):
        ...
        self.logger.info(f"Initialized OpenAI O3 Chat Agent: {agent_id}")
    ...
    def _setup_logging(self):
        ...
        # Create logger
        self.logger = logging.getLogger(f"OpenAIO3Agent_{self.agent_id}")
    ...
    def _get_api_key(self) -> str:
        ...
        if secrets_file.exists():
            try:
                with open(secrets_file, 'r') as f:
                    ...
                    api_key = keys.get(self.MODEL) or keys.get('default')
                    if api_key:
        ...
        print(f"{Fore.YELLOW}API key not found for OpenAI {self.MODEL_DISPLAY} model.")
        print(f"You can set the OPENAI_API_KEY environment variable or enter it now.{Style.RESET_ALL}")
        
        api_key = input(f"{Fore.CYAN}Enter API key for OpenAI {self.MODEL_DISPLAY}: {Style.RESET_ALL}").strip()
    ...
    def _build_api_payload(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Build the API request payload matching the O3 CURL structure"""
        # Process file inclusions
        processed_message = self._process_file_inclusions(new_message)
        
        # Build messages in the exact O3 CURL structure
        messages = []
        ...
        # Build payload matching the exact O3 CURL structure
        payload = {
            "model": self.MODEL,
            "messages": messages,
    ...
    def _make_api_request(self, payload: Dict[str, Any]) -> requests.Response:
        ...
        # Get appropriate timeout based on reasoning effort
        reasoning_effort = payload.get("reasoning_effort", "medium")
        timeout = AgentConfig.get_timeout_for_reasoning(reasoning_effort)
        
        self.logger.info(f"Using timeout of {timeout}s for {self.MODEL_DISPLAY} with reasoning effort: {reasoning_effort}")
        ...
        for attempt in range(max_retries):
            try:
                self.logger.info(f"Making API request to {self.MODEL_DISPLAY} (attempt {attempt + 1}/{max_retries}) with {timeout}s timeout...")
    ...
    def call_api(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Generator[str, None, None]:
        """Call OpenAI O3 API with the new message"""
        try:
            ...
            reasoning_effort = payload.get("reasoning_effort", "medium")
            timeout = self.config.get_timeout_for_reasoning(reasoning_effort)
            print(f"{Fore.YELLOW}🧠 Using {self.MODEL_DISPLAY} with {reasoning_effort.upper()} reasoning (timeout: {timeout//60}min {timeout%60}s)...{Style.RESET_ALL}")
    ...
    def export_conversation(self, format_type: str) -> str:
        ...
        if format_type == "json":
            ...
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(f"OpenAI {self.MODEL_DISPLAY} Chat Agent Conversation Export\n")
            ...
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(f"# OpenAI {self.MODEL_DISPLAY} Chat Agent Conversation\n\n")
                f.write(f"**Agent ID:** {self.agent_id}  \n")
                f.write(f"**Model:** {self.MODEL}  \n")
                f.write(f"**Exported:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  \n\n")
...
<head>
    ...
    <title>OpenAI {self.MODEL_DISPLAY} Conversation - {self.agent_id}</title>
    <style>
        :root {{
...
<body>
    <div class="container">
        ...
        <div class="footer">
            Generated by OpenAI {self.MODEL_DISPLAY} Chat Agent • Agent ID: {self.agent_id} • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
        </div>
    </div>
</body>
</html>"""
            ...
            if config_file.exists():
                try:
                    with open(config_file) as f:
                        ...
                        agent_info["model"] = "o3"  # Fixed to o3
...
def show_agent_info(agent_id: str):
    ...
    if config_file.exists():
        try:
            ...
            print(f"{Fore.WHITE}  Temperature: {config.get('temperature', 1.0)}")
...
def create_agent_config_interactive() -> AgentConfig:
    ...
    # Reasoning effort
    effort_input = input(f"Reasoning effort (low/medium/high) [{config.reasoning_effort}]: ").strip()
    if effort_input and effort_input in ["low", "medium", "high"]:
        config.reasoning_effort = effort_input
        
        # Show timeout for selected effort
        timeout = AgentConfig.get_timeout_for_reasoning(config.reasoning_effort)
        print(f"{Fore.YELLOW}  → Timeout with {config.reasoning_effort} effort: {timeout}s ({timeout//60}min {timeout%60}s){Style.RESET_ALL}")
    
    # Reasoning summary
    summary_input = input(f"Reasoning summary (auto/detailed/none) [{config.reasoning_summary}]: ").strip()
...
def interactive_chat(agent: OpenAIO3ChatAgent):
    """Interactive chat session"""
    print(f"\n{Fore.GREEN}Starting interactive chat with OpenAI {agent.MODEL_DISPLAY}")
        ...
        try:
            ...
            if user_input.startswith('/'):
                ...
                if command == 'help':
                    ...
                    print(f"{Fore.YELLOW}🧠 O3 Model & Reasoning Timeouts:")
                    print(f"{Fore.WHITE}  O3:")
                    print(f"    Low: 240s, Medium: 600s, High: 1200s")
                    print()
                    ...
                    print(f"{Fore.WHITE}Model: {agent.MODEL} ({agent.MODEL_DISPLAY})")
                    ...
                    for key, value in config_dict.items():
                        if key not in ['created_at', 'updated_at']:
                            if key == 'reasoning_effort':
                                timeout = AgentConfig.get_timeout_for_reasoning(str(value))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OpenAI O3 Chat Agent - Advanced Reasoning AI Chat Interface",
  ...
  %(prog)s --agent-id my-agent --config          # Configure agent interactively
    ...
    if args.list:
        ...
        for agent in agents:
            ...
            model = agent.get('model', 'o3')  # Fixed to o3
            model_display = "O3"  # Fixed display
    ...
    try:
        # Initialize agent
        agent = OpenAIO3ChatAgent(args.agent_id)
        ...
        if overrides:
            ...
            agent.config = AgentConfig(**config_dict)
            agent._save_config()
