#!/usr/bin/env python3
"""
Anthropic Claude Opus 4 Chat Agent - Production-Ready Chat Interface

Cette application fournit une implémentation complète d'un agent de chat utilisant le modèle Claude Opus 4 d'Anthropic
...
@dataclass
class AgentConfig:
    """Paramètres de configuration pour l'agent de chat Claude Opus 4 d'Anthropic"""
    model: str = "claude-opus-4-1-20250805"  # Modèle mis à jour
    temperature: float = 1.0
    max_tokens: Optional[int] = 32000  # Mis à jour
    max_history_size: int = 1000
    stream: bool = True
    system_prompt: Optional[str] = "you are agent"  # Mis à jour
    response_format: str = "json"  # Assurez-vous que le format de réponse est JSON
    top_p: float = 1.0
...
class AnthropicClaudeChatAgent:
    """Agent de chat Claude Opus 4 d'Anthropic avec persistance et support du streaming"""

    SUPPORTED_MODEL = {
        "claude-opus-4-1-20250805": {
            "name": "Claude Opus 4",
            "description": "Modèle Claude Opus 4 d'Anthropic",
            "timeout": 300,  # 5 minutes pour les très longues réponses
            "max_output_tokens": 32000  # Mis à jour
        },
    ...
    def _build_api_payload(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        # Exemple d'historique basé sur la requête curl fournie
        # Vous pouvez ajuster cela selon vos besoins
        # Ici, nous ajoutons les messages précédents avant d'ajouter le nouveau message
        for msg in self.messages:
            if msg["role"] in ["user", "assistant"]:
                # Convertir le contenu en liste de blocs de contenu
                content_blocks = []
        ...
        # Assurez-vous que max_tokens ne dépasse pas la limite du modèle
        max_tokens = min(config["max_tokens"], self.SUPPORTED_MODEL[self.config.model]["max_output_tokens"])
    ...
    def _make_api_request(self, payload: Dict[str, Any]) -> requests.Response:
        ...
        headers = {
            ...
            "anthropic-version": "2023-06-01"  # Version mise à jour
        }
    ...
    def _parse_streaming_response(self, response: requests.Response) -> Generator[str, None, None]:
        """Parser la réponse en streaming d'Anthropic et retourner le texte"""
        accumulated_text = ""
        try:
    ...
    def call_api(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Generator[str, None, None]:
        """Appeler l'API Claude Opus 4 d'Anthropic avec le nouveau message"""
        try:
...
"""
        ...
        # Générer les messages dynamiquement
        for msg in self.messages:
            timestamp = datetime.fromisoformat(msg["timestamp"]).strftime("%Y-%m-%d %H:%M:%S")
            role = msg["role"]
            content = msg["content"]
    ...
    def interactive_chat(self):
        ...
        print(f"\n{Fore.GREEN}Démarrage du chat interactif avec Claude Opus 4 d'Anthropic")
                ...
                if config_file.exists():
                    try:
                        with open(config_file) as f:
                            ...
                            agent_info["model"] = config.get("model", "claude-opus-4-1-20250805")
    ...
    def show_agent_info(agent_id: str):
        ...
        if config_file.exists():
            try:
                ...
                model = config.get('model', 'claude-opus-4-1-20250805')
    ...
    def main():
        ...
        parser = argparse.ArgumentParser(
            description="Anthropic Claude Opus 4 Chat Agent - Advanced AI Chat Interface",
  ...
  %(prog)s --agent-id mon-agent --config          # Configure agent interactively
        ...
        if args.list:
            ...
            for agent in agents:
                ...
                model = agent.get('model', 'claude-opus-4-1-20250805')
                model_display = AnthropicClaudeChatAgent.SUPPORTED_MODEL.get(model, {}).get('name', model)
