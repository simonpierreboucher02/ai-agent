#!/usr/bin/env python3
"""
Anthropic Claude 3.7 Chat Agent - Production-Ready Chat Interface

Cette application fournit une implémentation complète d'un agent de chat utilisant le modèle Claude 3.7 d'Anthropic
...
@dataclass
class AgentConfig:
    """Paramètres de configuration pour l'agent de chat Claude 3.7 d'Anthropic"""
    model: str = "claude-3-7-sonnet-20250219"  # Modèle par défaut de Claude 3.7
    temperature: float = 1.0
    max_tokens: Optional[int] = 128000  # Mise à jour de 4096 à 128000
...
class AnthropicClaudeChatAgent:
    """Agent de chat Claude 3.7 d'Anthropic avec persistance et support du streaming"""
    ...
    SUPPORTED_MODEL = {
        "claude-3-7-sonnet-20250219": {
            ...
            "max_output_tokens": 128000  # Mise à jour de 64000 à 128000
        },
    ...
    def _make_api_request(self, payload: Dict[str, Any]) -> requests.Response:
        ...
        headers = {
            ...
            "anthropic-beta": "output-128k-2025-02-19"  # Ajout de l'en-tête beta
        }
    ...
    def call_api(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Generator[str, None, None]:
        """Appeler l'API Claude 3.7 d'Anthropic avec le nouveau message"""
        try:
...
"""
    ...
    def interactive_chat(self):
        ...
        print(f"\n{Fore.GREEN}Démarrage du chat interactif avec Claude 3.7 d'Anthropic")
                ...
                if config_file.exists():
                    try:
                        with open(config_file) as f:
                            ...
                            agent_info["model"] = config.get("model", "claude-3-7-sonnet-20250219")
    ...
    def show_agent_info(agent_id: str):
        ...
        if config_file.exists():
            try:
                ...
                print(f"  Max Tokens: {config.get('max_tokens', 128000)}")  # Mise à jour ici
    ...
    def main():
        ...
        parser = argparse.ArgumentParser(
            description="Anthropic Claude 3.7 Chat Agent - Advanced AI Chat Interface",
  ...
  %(prog)s --agent-id mon-agent --config          # Configure agent interactively
        ...
        if args.list:
            ...
            for agent in agents:
                ...
                model = agent.get('model', 'claude-3-7-sonnet-20250219')
                model_display = AnthropicClaudeChatAgent.SUPPORTED_MODEL.get(model, {}).get('name', model)
